...
Currently supports: **Slack**, **Heartbeat**, **HTTP Webhook** + **Claude Code**, **Pi**, **Codex**, **Opencode**
...
| `AGENT_BRIDGE_PROFILES_PATH` | No | — (disabled) | TOML file with named agent profiles (`[claude.profiles.*]`, `[pi.profiles.*]`, `[codex.profiles.*]`, `[opencode.profiles.*]`) + Slack channel→profile routing (see `profiles.example.toml`) |
...
| `AGENT_BRIDGE_CODEX_*` | No | — | Base config for named [Codex agent](docs/agents/codex.md) profiles (work dir, sandbox mode, model, effort, session map path) |
| `AGENT_BRIDGE_OPENCODE_*` | No | — | Base config for named [Opencode agent](docs/agents/opencode.md) profiles (work dir, model, variant, session map path) |
