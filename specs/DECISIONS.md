...
- **D44 [T5]** API context 合併：adapter 注入的 `platform: api` 覆蓋 client 傳入的同名鍵（其餘 client 鍵原樣通過）— platform 麵包屑必須可信，caller 不得偽裝成其他平台。API 無 sender identity → prompt 不加前綴、原文轉送（adding-platform 準則第 5 點的「無身分」分支）。
- **D45 [T5]** Buffered 回應以 `Completion.text` 為權威最終文字（緩衝的 TextDelta 僅在其為空時 fallback），對齊 Slack 語義；`questions` 欄位僅在 agent 發出 `UserQuestion` 時附加於回應（一般流程 schema 不變、不遺失提問內容），client 以同 `session` 續 POST 作答。
- **D46 [T5]** capacity_full 以 `Completion.metadata["error_code"] == "capacity_full"` 判斷 → HTTP 503、body 與 200 回應同構；SSE 模式先 peek 第一個 bridge 事件（bridge 短路路徑只 yield 一個 error Completion、無 Processing），是 capacity 就直接回純 503 JSON，不開一個只有單一事件的 stream。
- **D47 [T5]** SSE 不送 keep-alive/heartbeat comment（run 受 agent timeout 上限，額外心跳只添噪音）；client 中途斷線 → 記 log、標記 disconnected 後**繼續 silent drain** bridge 事件至完成 — agent run 正常結束、semaphore 正常釋放、session 狀態一致（之後可用同 session 取回結果脈絡），不 cancel、不 raise。
- **D48 [T5]** per-session lock 僅施於 client session（`api:client:*`，同 session 第二個 POST 等待而非競速 resume）；oneshot 不加鎖 — bridge 全域 semaphore 已擋整體併發，adapter 不重複限流。lock dict 走 `cleanup_stale_sessions()` hook 回收（D38 模式）。`stop()` 不需 LINE 式背景任務收斂 — 所有工作都在 request handler 內，改以 AppRunner `shutdown_timeout=5s` 讓 aiohttp graceful shutdown 等待 in-flight 請求。
- **D49 [T6]** Codex CLI 查證基準：本機 npm `@openai/codex` 0.130.0（殼層存在但原生二進位缺失，`--help` 無法執行），介面依官方文件 `developers.openai.com/codex/noninteractive` 與官方 TypeScript SDK 型別定義（github.com/openai/codex `sdk/typescript` 的 ThreadEvent/ThreadItem union，直接解析同一份 `codex exec --json` 輸出）查證：新 run `codex exec [flags] "<prompt>"`、續聊 `codex exec resume <thread_id> [flags] "<prompt>"`、`--json` JSONL（`thread.started`/`turn.started`/`item.started|updated|completed`/`turn.completed`/`turn.failed`/`error`；item 型別 `agent_message`/`reasoning`/`command_execution`/`file_change`/`mcp_tool_call`/`web_search`/`todo_list`/`error`）、`--sandbox {read-only,workspace-write,danger-full-access}`、`-m/--model`、`--skip-git-repo-check`。版本假設記入 `docs/agents/codex.md`。
- **D50 [T6]** 恆帶 `--skip-git-repo-check` — `codex exec` 預設拒絕在非 git 目錄執行，而 bridge 的 work_dir 不必然是 repo（測試亦用 tmp 目錄）；bridge 部署形態下的防護由 `--sandbox` 承擔，git 檢查僅添啟動失敗面。
- **D51 [T6]** `system_prompt` 前置格式（D10 落地）：`codex exec` 無穩定等價旗標（instructions 類 config override 屬實驗性），以 `<platform-directives>\n{system_prompt}\n</platform-directives>\n\n{prompt}` 前置 — 標籤分隔讓模型可區分平台指令與使用者訊息；空/None 時 prompt 原樣傳遞。格式文件化於 `docs/agents/codex.md`。
- **D52 [T6]** Codex session 映射檔為扁平 JSON 物件 `{bridge_session_id: codex_thread_id}`（`CodexSessionMap`，預設 `./codex-sessions.json`）：載入失敗（壞 JSON／非物件）→ warn + 空映射起步；寫入失敗 → error log、記憶體狀態保留不 crash（最壞情況重啟後失去 resume 續航，落入 D53 fallback）。`cleanup_session` 移除對應條目、never raises。
- **D53 [T6]** resume 查無映射（重啟丟檔、首寫前 crash）→ log warning、以全新 codex thread 執行並把新 thread_id 寫回映射 — 對話脈絡遺失但訊息照常服務，Completion 不標 error（服務可用性優先於脈絡完整性）。
- **D54 [T6]** usage 映射：Codex 回報 OpenAI 語意（`input_tokens` 內含 `cached_input_tokens` 子集），bridge canonical keys 要求 input/output 排除 cache → `input_tokens = max(input−cached, 0)`、`cache_read_tokens = cached`、`cache_creation_tokens = 0`（無此概念）、`num_turns = 1`（exec 一次一 turn）。CLI 不報 cost → `cost_usd` 維持 0；CLI 不報 duration → `duration_ms` 由 controller 以 wall-clock 自量。
- **D55 [T6]** 終態事件為 `turn.completed`／`turn.failed`／`error` 三者任一（讀到即停止讀取，鏡射 claude 的 result 短路防孤兒管線）。`turn.completed` 只帶 usage 無文字 → `Completion.text` 由 controller 以最後一則 `agent_message` 補上（與平台看到的最終 TextDelta 同源）。事件映射：`agent_message`(item.completed) → TextDelta；`command_execution`/`mcp_tool_call`/`web_search`/`file_change`(item.started) → StatusUpdate；`reasoning`/`todo_list`/`item.updated` → 內部過濾；未知 event/item 型別 → log + skip（D9）。
