...
async def test_run_breaks_on_result_despite_orphan_holding_stdout(
    ...
    controller = ClaudeController(ClaudeConfig(work_dir=tmp_path, timeout_seconds=5.0))
...
async def test_run_expired_deadline_fails_before_first_read(
    fake_claude: FakeClaudeFactory,
):
    # Budget so small the deadline is already behind us when the read loop
    # takes its first pass — exercises the `remaining <= 0` guard.
    cli = fake_claude(claude_cli.reply_steps("never"), timeout_seconds=1e-9)
    controller = ClaudeController(cli.config)
    events = [e async for e in controller.run("s1", "hi", is_new=True)]
    completion = events[-1]
    assert isinstance(completion, Completion)
